//
//  VideoChatVC_Vidyo.swift
//  TelePatriot
//
//  Created by Brent Dunklau on 8/10/18.
...
class VideoChatVC_Vidyo: BaseViewController
//    , VCConnectorIConnect, VCConnectorIRegisterRemoteCameraEventListener,
...
//
//        local_camera_view = UIView(frame: CGRect(x: 0, y: localCameraYPos, width: localCameraWidth, height: localHeight))
//        remoteViews = UIView(frame: CGRect(x: remoteXPos, y: remoteYPos, width: remoteWidth, height: remoteHeight))
//
//        // init the spinner here, then showSpinner() whenever we need it
//        getSpinner()
//
//        // Vidyo code ref:   https://vidyo.io/blog/how-to/vidyo-io-using-swift-build-ios-video-chat-app/
...
//
//
//        local_camera_view?.backgroundColor = UIColor.red
//        view.addSubview(local_camera_view!) // placement is dictated by the dimensions of the CGRect in the UIView's constructor
//
//
...
//        // If I'm coming here from an invitation, the object will be nil but I don't want to create another video node
//        // Shouldn't the node I accepted by written to my account at this point?  So that I can fall through here one time
//        // but just get the node I'm associated with in getVideoNodeKey() below ?
//        if videoNode != nil {
//            return // only do the query below if the videoNode object is nil
//        }
//        
...
////                }
//                
//                // controls the connect_button - what image does it display...  what actions are taken client-side
...
//                    }
//                    else {
//                        self.connect_button.setImage(UIImage(named: "callStart.png"), for: UIControlState.normal)
//                        self.doDisconnect()
//                    }
//                }
//                
...
//                        self.showSpinner2()
//                        self.simpleOKDialog(message: "The recording will start in 30-45 seconds.  When the spinner goes away, the recording will begin.")
//                    } // spinner dismissed in recordingHasStarted()
//                }
//                
...
//            self.recording = true
//            self.record_button.setImage(UIImage(named: "recordstop.png"), for: UIControlState.normal)
//            self.publish_button.isHidden = true
//            self.dismissSpinner()
//        }
//    }
//    
...
//        if let video_node_key = videoNode?.getKey(),
//            let video_participant = videoNode?.video_participants[TPUser.sharedInstance.getUid()],
//            let room_id = videoNode?.getKey() // room_id is not a field in VideoNode (maybe an oversight)
//        {
//            //getSpinner()
//            showSpinner() // dismissed in doConnect() and doDisconnect()
//            var request_type = "connect request"
//            if video_participant.isConnected() {
//                request_type = "disconnect request"
//            }
//            
//            let ve = VideoEvent(user: TPUser.sharedInstance, video_node_key: video_node_key, room_id: room_id, request_type: request_type)
//            ve.save()
//        }
//        
//        
...
//        guard let vn = videoNode, let _ = vn.legislator else {
...
//        // TODO whenever we put this back in, we have to connect using "connect request" calls now
//        //        if(!connected) {
//        //            doConnect();
//        //        }
...
//        // set the button image to callEnd immediately so the user gets instant feedback.  If there's an actual
//        // problem disconnecting, the live query in
//        connector?.disconnect()
//        DispatchQueue.main.async {
//            self.record_button.isHidden = true
//            self.dismissSpinner()
...
//    
//    /****
//     Rethink connecting to Vidyo:  Don't just connect; make a connection request
//     Connection should first be requested - write a "connection request" record to /video/video_events
//     Then have a trigger respond to that event and write "connected:(date)" on the video participant's node
//     The client-side query of the video node will pick up this connect_date:date attribute and will call doConnect()
//     as a result
//     Likewise for disconnects, write a "disconnect request" to video/video_events and have a trigger respond to that.
//     The trigger will write disconnect_date:date to the participant's record.  And the live query will pick up THAT
//     attribute and call disconnect client-side.
//     
//     *****/
//    private func doConnect() {
//        if (connector?.getState() == VCConnectorState.connected || connector?.getState() == VCConnectorState.establishingConnection) {
//            return
//        }
//        
...
//        DispatchQueue.main.async {
//            
//            guard let w = self.local_camera_view?.frame.size.width,
//                let h = self.local_camera_view?.frame.size.height else {
//                    return
//            }
//            
...
//            self.remoteCameraVisible = true
//            self.refreshUI()
//            self.inviteLinks() // maybe could put this inside refreshUI() - don't know yet 8/7/18
//        }
//    }
//    
//    func onRemoteCameraRemoved(_ remoteCamera: VCRemoteCamera!, participant: VCParticipant!) {
//        self.remoteCameraVisible = false
//        numberOfRemoteViews -= 1 // pointless, there can only be 1 or 0
//        DispatchQueue.main.async {
...
//        self.local_camera_view?.isHidden = true
//    }
//    
//    // Equiv in Android is VidyoChatFragment.onLocalCameraAdded()
//    func onLocalCameraAdded(_ localCamera: VCLocalCamera!) {
//        if ((localCamera) != nil) {
//            self.local_camera_view?.isHidden = false
//            localCamera.setPreviewLabel(TPUser.sharedInstance.getName())
//            DispatchQueue.main.async {
//                
//                if let w = self.local_camera_view?.frame.size.width, let h = self.local_camera_view?.frame.size.height {
//                    self.connector?.assignView(toLocalCamera: UnsafeMutableRawPointer(&self.local_camera_view),
//                                               localCamera: localCamera,
//                                               displayCropped: false,
//                                               allowZoom: false)
//                    self.connector?.showViewLabel(UnsafeMutableRawPointer(&self.local_camera_view),
//                                                  showLabel: false)
//                    self.connector?.showView(at: UnsafeMutableRawPointer(&self.local_camera_view),
//                                             x: 0,
//                                             y: 0,
//                                             width: UInt32(w),
//                                             height: UInt32(h))
